...
from application.dto.task_dto import TaskCreateDTO, TaskUpdateDTO, TaskResponseDTO, TaskListResponseDTO
...
def get_task_repository(session: AsyncSession = Depends(get_session)) -> TaskRepositoryImpl:
...
async def get_tasks(
    ...
    try:
        ...
        if completed is not None:
            if completed:
                tasks = await task_repository.get_completed_by_user_id(current_user.id, page_size, offset)
            else:
                tasks = await task_repository.get_pending_by_user_id(current_user.id, page_size, offset)
        else:
            tasks = await task_repository.get_by_user_id(current_user.id, page_size, offset)
